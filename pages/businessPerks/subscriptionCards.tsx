...
const subscriptionCards = (
  { iconImage, desc, id, isCardId, isAllCardActive }: Props,
  props: any
) => {
    ...
    <div
      className={`${
        (isActive && cardId === id) || (isActive && isAllActive)
          ? 'flex flex-col w-[255px] h-[213px] bg-[white] rounded-xlx pl-8 pt-4 shadow-smBoxWhite'
          : 'flex flex-col w-[255px] h-[213px] bg-[white] rounded-xlx pl-8 pt-4 shadow-smBoxWhite opacity-20'
      }`}
      id="all"
    >
      ...
      <Link href="/subscribeScreens">
        <div className="flex flex-row items-center justify-center p-2 h-[38px] w-[166px] rounded-sm border-1 border-secondGreen mt-[25px] cursor-pointer">
